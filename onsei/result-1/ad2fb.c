...
)
{
  ...
  for(i_frame=0; i_frame<n_frame; i_frame++) {
    ...
    /*-------------------------------------------------------
      ...
      したがって, (3.1)式,(3.2),(3.3)式と
      この関数の変数の関係は次のようになる。
      ----------------- ------------------- --------------
      (3.1)(3.2)(3.3)式 この関数            変数の型
      ----------------- ------------------- --------------
      i                 i                   整数 int
      j                 j                   整数 int
      k                 k                   整数 int
      ...
      Aj                Aj                  浮動小数点 float
      ----------------- ------------------- --------------
      添字 i_frame は(3.1)(3.2)(3.3)式に関係ないが,
      上記の対応となるので注意。
...
　    あらかじめ　s = 0; という代入文で変数sの値を
	...
	W_kj = (double)(k - klo[j]) / (double)(kc[j] - klo[j]); /* ここを穴埋め（済） */
	m[i_frame][j] += W_kj * X[k] * X[k]; /* ここを穴埋め（済） */
	Aj += W_kj; /* ここを穴埋め（済） */
      }
      for(k=kc[j]; k<=khi[j]; k++) {
	W_kj = (double)(khi[j] - k) / (double)(khi[j] - kc[j]); /* ここを穴埋め（済） */
	m[i_frame][j] += W_kj * X[k] * X[k]; /* ここを穴埋め（済） */
	Aj += W_kj; /* ここを穴埋め（済） */
      }
      /* フィルタの面積が等しくなるように正規化 */
      m[i_frame][j] /= (double)Aj;
    }
  }
